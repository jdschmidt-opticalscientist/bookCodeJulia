...
p1 = heatmap(x_vec, x_vec, C_cont,
    aspect_ratio=:equal, title="Analytic",
    xlabel="x [m]", ylabel="y [m]",
    xlims=(-L / 2, L / 2), ylims=(-L / 2, L / 2))

# Top Right: Numerical Heatmap
p2 = heatmap(x_vec, x_vec, real.(C),
    aspect_ratio=:equal, title="Numerical",
    xlabel="x [m]", ylabel="y [m]",
    xlims=(-L / 2, L / 2), ylims=(-L / 2, L / 2))
...
p3 = plot(x_vec, slice_ana,
    color=:red, marker=:square, ms=3, lw=1.5,
    label="Analytic",
    title="Cross-section comparison at y=0",
    xlabel="x [m]", ylabel="Amplitude",
    grid=true)

scatter!(p3, x_vec, slice_num,
