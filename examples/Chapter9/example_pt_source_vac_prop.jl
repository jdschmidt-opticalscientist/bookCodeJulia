...
p1 = heatmap(xn[1, :] .* 1e3, yn[:, 1] .* 1e3, Ivac,
    aspect_ratio=:equal, color=:inferno,
    title="Vacuum Irradiance", xlabel="x [mm]", ylabel="y [mm]")

p2 = heatmap(xn[1, :] .* 1e3, yn[:, 1] .* 1e3, angle.(Uvac),
    aspect_ratio=:equal, color=:phase,
    title="Vacuum Phase", xlabel="x [mm]", ylabel="y [mm]")
