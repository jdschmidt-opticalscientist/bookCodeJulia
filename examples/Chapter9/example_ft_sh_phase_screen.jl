...
# --- Visualization ---
p1 = heatmap(x, y, phz_hi,
    aspect_ratio=:equal, c=:viridis,
    title="High Freq (FFT only)", xlabel="x [m]", ylabel="y [m]")

p2 = heatmap(x, y, phz_lo,
    aspect_ratio=:equal, c=:viridis,
    title="Low Freq (Subharmonics)", xlabel="x [m]", ylabel="y [m]")

p3 = heatmap(x, y, phz,
    aspect_ratio=:equal, c=:viridis,
    title="Combined Phase Screen", xlabel="x [m]", ylabel="y [m]")
